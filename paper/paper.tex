...
\noindent\textbf{MSC 2020:} 11B83, 11Y16, 37A45, 60C05
...
application of~$T$ eventually reaches~$1$. The problem has a long history
(see the survey by Lagarias~\cite{Lagarias1985} and the early computational
study by Crandall~\cite{Crandall1978}); despite substantial progress ---
...
\begin{enumerate}
  ...
  \item A \emph{prefix--suffix decomposition} revealing that the low ratio
    is primarily a suffix effect: the prefix-only ratio is $0.853$, near the
...
sorting algorithm in $O(k \log k)$ time (see Aldous and
Diaconis~\cite{AldousDiaconis1999} for background on patience sorting and
its connection to the BDJ theorem). Correctness is verified by
...
Bootstrap confidence intervals~\cite{Efron1979} use 1000 resamples. We note
that trajectory
...
% Collatz ratio
\addplot[thick, mark=*, blue!80!black] coordinates {
  (1000, 0.632)
  (5000, 0.627)
  (10000, 0.621)
  (50000, 0.599)
...
\addlegendentry{Collatz $\bar{r}(N)$}

% Random baseline
\addplot[thick, dashed, red!70!black, mark=none] coordinates {
  (500, 0.812)
  (2000000, 0.812)
};
\addlegendentry{Random baseline ($0.812$)}

% Bernoulli baseline
\addplot[thick, dotted, green!50!black, mark=none] coordinates {
  (500, 0.571)
  (2000000, 0.571)
};
\addlegendentry{Bernoulli $p = 0.316$ ($0.571$)}

\end{semilogxaxis}
\end{tikzpicture}
\caption{Mean LIS ratio $\bar{r}(N)$ versus $N$ on a log scale. The
Collatz ratio (solid, circles) decreases monotonically and has not
converged at $N = 10^6$. The random baseline (dashed) and the Bernoulli
model with $p = 0.316$ (dotted) are shown for reference.}
\label{fig:scaling}
\end{figure}

The ratio is monotonically decreasing over the measured range
(\cref{fig:scaling}). The inter-step change $\Delta$ is diminishing but
has not reached zero: at $N = 10^6$ the ratio is still falling by $0.003$
per doubling step. The ratio has \emph{not} converged at $N = 10^6$.
...
\begin{table}[ht]
\centering
\caption{LIS ratio for Collatz and null models at $N = 50{,}000$. Each
model adds one structural feature beyond the previous. The ``Gap'' column
shows the difference from the Collatz ratio at the same~$N$.}
\label{tab:null}
\begin{tabular}{@{}llcc@{}}
\toprule
...
\noindent
\emph{Note on $N$ values.} The null model comparisons in
\cref{tab:null} use $N = 50{,}000$ so that all models (including the
computationally expensive Markov simulation) run at the same scale.
The Collatz ratio at this $N$ is $0.593$; at $N = 10^6$
(\cref{tab:scaling}) it has decreased to $0.567$. The relative ordering
and approximate gaps between models are stable across $N$.
...
\begin{remark}
The classical heuristic for the ``shortcut'' Collatz map
$(3n+1)/2$ predicts that each shortcut step increases with probability
$\log_2 3 \approx 0.585$~\cite{Lagarias1985}. Our $31.6\%$ is not in
conflict: we count \emph{individual} steps of the original map~\eqref{eq:collatz},
where each odd step $n \mapsto 3n+1$ (increasing) is necessarily followed by
at least one even step $n \mapsto n/2$ (decreasing). The fraction of odd
steps among all steps is approximately $1/(1 + \log_2 3) \approx 0.387$
under the classical heuristic; the empirical $0.316$ is somewhat lower,
consistent with the observation that $3n+1$ often produces numbers
divisible by~$4$ or higher powers of~$2$, yielding multiple consecutive
halvings per odd step.
\end{remark}
...
\begin{table}[ht]
\centering
\caption{LIS ratio for the $3n+b$ family ($N = 50{,}000$). Only converging
trajectories are included. $^\dagger$Based on $\sim\!15$ converging
trajectories; unreliable.}
\label{tab:parameter}
\begin{tabular}{@{}ccccc@{}}
\toprule
...
Excluding the unreliable $b = 3$ row ($\sim\!15$ converging trajectories),
the ratio varies from $0.489$ ($b = 7$) to $0.599$ ($b = 1$), a spread of
$0.11$. This suggests that the LIS ratio encodes dynamical information
specific to each map variant, not merely a universal consequence of the
$3n+b$ structure.

We note two caveats. First, the low convergence rate for $b \neq 1$
introduces selection bias: only trajectories that reach a cycle within
$10^5$ steps are included, and these may not be representative of typical
orbits. Second, the $b = 3$ row is based on too few trajectories to be
meaningful. The parameter sensitivity observation should be treated as
...
Bootstrap confidence intervals assume independent samples. With effective
sample size $N_{\mathrm{eff}} \approx 500$ out of $N = 20{,}000$, the
standard error is underestimated by a factor of
$\sqrt{N / N_{\mathrm{eff}}} = \sqrt{20{,}000 / 500} \approx 6.3$.
The naive bootstrap CI width of $0.003$ at $N = 10^6$ should therefore
be widened by roughly this factor, giving an adjusted $95\%$ CI for
$\bar{r}(10^6)$ of approximately $[0.55, 0.59]$.
